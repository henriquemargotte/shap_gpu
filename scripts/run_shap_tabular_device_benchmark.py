...
def run_explainer_and_time(wrapper, X, background=None):
    # Create a simple Independent masker using the provided background
    masker = None
...
def main():
    ...
    results = {"n_samples": n, "dataset": "breast_cancer", "cpu": {}, "gpu": {}, "cuml_native": {}}
    ...
    vals_cpu, total_cpu = run_explainer_and_time(cpu_wrapper, X_subset, background=X_train[:100])
    ...
    if gpu_available:
        ...
        vals_gpu, total_gpu = run_explainer_and_time(gpu_wrapper, X_subset, background=X_train[:100])
        ...
        if has_cuml_explainer and cp is not None:
            try:
                ...
                # prepare GPU arrays (float32) for cuML explainer
